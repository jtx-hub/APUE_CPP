...
void test_move_and_copy() {
    ...
    FileGuard file("test_move.txt", O_CREAT | O_WRONLY | O_TRUNC, 0644);
    file.Write("1. Hello from original file!\n", 29);

    /* --- 极度危险的拷贝测试 (请取消注释尝试编译) --- */
    // 如果你的 FileGuard 写得够好，下面两行代码【必须导致编译失败】！
    // 因为两个 fd 析构时会 close 两次，造成 Double Free 灾难。
    // FileGuard file_copy = file;            // 报错：使用了被删除的函数
    // FileGuard file_assign; file_assign = file; // 报错：使用了被删除的函数
...
void test_open_failure() {
    ...
    // 故意写一个绝对不存在的乱码目录
    FileGuard file("/invalid_dir_name_888/test.txt", O_CREAT | O_WRONLY, 0644);
...
void test_stack_unwinding() {
    ...
    FileGuard file("test_unwind.txt", O_CREAT | O_WRONLY | O_TRUNC, 0644);
...
// ==========================================
int main() {
    std::cout << "============= FileGuard 工业级压力测试 =============" << std::endl;
